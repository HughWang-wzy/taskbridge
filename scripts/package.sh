...
./scripts/build.sh
mkdir -p release
rm -f release/*.tar.gz release/*.zip release/install.sh release/install.ps1 release/SHA256SUMS
staging="$(mktemp -d)"
...
(cd release && "${checksum_command[@]}" ./*.tar.gz ./*.zip ./install.sh ./install.ps1 > SHA256SUMS)
