...
def main():
    with tempfile.TemporaryDirectory() as temporary:
        ...
        command("git", "tag", "v0.4.1", cwd=source)
        ...
        command("git", "clone", "--branch", "v0.4.1", str(remote), str(checkout))
        ...
        assert command("git", "rev-parse", "HEAD", cwd=checkout) == command("git", "rev-list", "-n", "1", "v0.4.2", cwd=source)
